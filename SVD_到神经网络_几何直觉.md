...
严格来说 $`\mathrm{rank}(A)=3`$，但第三个方向几乎不起作用。工程上，我们很可能直接使用
...
```math
\mathrm{ReLU}(W_1x+b_1)=D(W_1x+b_1).
```

若后面再接一层线性输出，

```math
f(x)=W_2\mathrm{ReLU}(W_1x+b_1)+b_2,
```
...
```math
a\,\mathrm{ReLU}(w^{\top}x)
=\frac{a}{c}\mathrm{ReLU}(cw^{\top}x),\qquad c>0.
```
...
精确可分离要求 $`\mathrm{rank}(F)=1`$，也就是只有一个奇异值非零。若后续奇异值只是很小，只能说这个卷积核近似可分离。
